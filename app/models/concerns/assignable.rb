module Assignable
  ...
  included do
    ...
    #
    # # the current assignment must be active (open), a root assignment (no
    # # parent) and it must belong to the model
    # (and not to a field of the model).
    # # There must always be exactly one current_assignment for each assignable.
    ...
    #   # return nil if the required field is not existing
    # on the assignable model
    ...
    # def create_new_assignment! creator_id, creator_team_id, receiver_id,
    # receiver_team_id, message = ''
